...
public:
    ...
    LayoutRect collectSelectionRectsForLineBoxes(const RenderElement* repaintContainer, bool clipToVisibleContent, Vector<LayoutRect>&);
...
private:
    ...
    LayoutRect selectionRectForRepaint(const RenderElement* repaintContainer, bool clipToVisibleContent = true) final;
    LayoutRect localCaretRect(InlineBox*, unsigned caretOffset, LayoutUnit* extraWidthToEndOfLine = nullptr) override;
    LayoutRect clippedOverflowRectForRepaint(const RenderElement* repaintContainer) const final;
    ...
    void container(const RenderElement&, bool&) const = delete; // Use parent() instead.
