...
std::pair<unsigned, unsigned> InlineTextBox::selectionStartEnd() const
{
    ...
    auto start = 0; //renderer().view().selection().startPosition();
    auto end = 9; //renderer().view().selection().endPosition();
...
int InlineTextBox::offsetForPosition(float lineOffset, bool includePartialGlyphs) const
{
    ...
    return 0;//lineFont().offsetForPosition(createTextRun(text), lineOffset - logicalLeft(), includePartialGlyphs);
}
