...
public:
    ...
    void getSubresourceURLs(ListHashSet<URL>&) const;
     ...
     ExceptionOr<void> removeChild(Node& child);
     ExceptionOr<void> appendChild(Node& newChild);
    ...
    Ref<Node> cloneNode(bool deep) { return cloneNodeInternal(document(), deep ? CloningOperation::Everything : CloningOperation::OnlySelf); }
     ExceptionOr<Ref<Node>> cloneNodeForBindings(bool deep);
    ...
    bool isSameNode(Node* other) const { return this == other; }
     ...
     ExceptionOr<void> setTextContent(const String&);
    ...
    enum class ShouldUpdateStyle { Update, DoNotUpdate };
     Editability computeEditability(UserSelectAllTreatment, ShouldUpdateStyle) const;

     unsigned computeNodeIndex() const;
