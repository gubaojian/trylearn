...
public:
    ...
    using DocumentsMap = HashMap<DocumentIdentifier, Document*>;
     static DocumentsMap::ValuesIteratorRange allDocuments();
     static DocumentsMap& allDocumentsMap();
    ...
    bool hasManifest() const;
     ...
     ExceptionOr<Ref<Element>> createElementForBindings(const AtomicString& tagName);
     Ref<DocumentFragment> createDocumentFragment();
     Ref<Text> createTextNode(const String& data);
     Ref<Comment> createComment(const String& data);
     ExceptionOr<Ref<CDATASection>> createCDATASection(const String& data);
     ExceptionOr<Ref<ProcessingInstruction>> createProcessingInstruction(const String& target, const String& data);
     ExceptionOr<Ref<Attr>> createAttribute(const String& name);
     ExceptionOr<Ref<Attr>> createAttributeNS(const AtomicString& namespaceURI, const String& qualifiedName, bool shouldIgnoreNamespaceChecks = false);
     ExceptionOr<Ref<Node>> importNode(Node& nodeToImport, bool deep);
     ExceptionOr<Ref<Element>> createElementNS(const AtomicString& namespaceURI, const String& qualifiedName);
     Ref<Element> createElement(const QualifiedName&, bool createdByParser);
    ...
    ReadyState readyState() const { return m_readyState; }
     ...
     String defaultCharsetForLegacyBindings() const;
    ...
    void setXMLEncoding(const String& encoding) { m_xmlEncoding = encoding; } // read-only property, only to be set from XMLDocumentParser
     ExceptionOr<void> setXMLVersion(const String&);
    ...
    bool isTimerThrottlingEnabled() const { return m_isTimerThrottlingEnabled; }
     ...
     ExceptionOr<Ref<Node>> adoptNode(Node& source);
    ...
    enum class RunPostLayoutTasks {
     ...
     void updateLayoutIgnorePendingStylesheets(RunPostLayoutTasks = RunPostLayoutTasks::Asynchronously);
    ...
    bool visuallyOrdered() const { return m_visuallyOrdered; }
     ...
     ExceptionOr<RefPtr<DOMWindow>> openForBindings(DOMWindow& activeWindow, DOMWindow& firstWindow, const String& url, const AtomicString& name, const String& features);
     ExceptionOr<Document&> openForBindings(Document* responsibleDocument, const String& type, const String& replace);

    // FIXME: We should rename this at some point and give back the name 'open' to the HTML specified ones.
     void open(Document* responsibleDocument = nullptr);
    void implicitOpen();
    ...
    void write(Document* responsibleDocument, SegmentedString&&);
     ExceptionOr<void> write(Document* responsibleDocument, Vector<String>&&);
     ExceptionOr<void> writeln(Document* responsibleDocument, Vector<String>&&);
    ...
    enum class FocusRemovalEventsMode { Dispatch, DoNotDispatch };
     bool setFocusedElement(Element*, FocusDirection = FocusDirectionNone,
    ...
    EventListener* getWindowAttributeEventListener(const AtomicString& eventType, DOMWrapperWorld&);
     void dispatchWindowEvent(Event&, EventTarget* = nullptr);
    void dispatchWindowLoadEvent();

     ExceptionOr<Ref<Event>> createEvent(const String& eventType);
    ...
    void titleElementTextChanged(Element& titleElement);
     ...
     ExceptionOr<String> cookie();
     ExceptionOr<void> setCookie(const String&);
    ...
    static bool hasValidNamespaceForAttributes(const QualifiedName&);
     ...
     HTMLElement* bodyOrFrameset() const;
     ExceptionOr<void> setBodyOrFrameset(RefPtr<HTMLElement>&&);
...
#include <WebKitAdditions/DocumentIOS.h>
    ...
    // Used for testing. Count handlers in the main document, and one per frame which contains handlers.
     unsigned wheelEventHandlerCount() const;
     unsigned touchEventHandlerCount() const;
    ...
    LayoutRect absoluteEventHandlerBounds(bool&);
    ...
    // Callers should try to create the ConsoleMessage themselves.
     //void addConsoleMessage(MessageSource, MessageLevel, const String& message, unsigned long requestIdentifier = 0) final;
    ...
    void updateViewportUnitsOnResize();
     ...
     void addAudioProducer(MediaProducer*);
     void removeAudioProducer(MediaProducer*);
    ...
    bool isCapturing() const { return MediaProducer::isCapturing(m_mediaState); }
     void updateIsPlayingMedia(uint64_t = HTMLMediaElementInvalidID);
...
// FIXME: Find a better place for this functionality.
    ...
    OrientationNotifier& orientationNotifier() { return m_orientationNotifier; }
     ...
     const AtomicString& linkColorForBindings() const;
     void setLinkColorForBindings(const String&);
    ...
    void didRemoveAttachmentElement(HTMLAttachmentElement&);
     RefPtr<HTMLAttachmentElement> attachmentForIdentifier(const String& identifier) const;
#endif
